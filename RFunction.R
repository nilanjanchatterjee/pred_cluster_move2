...
######################################################################################
rFunction <-function(data, sdk, search_radius, window_days, clus_min_locs, 
                     centroid_calc="mean",daylight_hrs=NA ){
  ...
  data_move <- mt_as_move2(dat,
                           coords = c("location_long", "location_lat"),
                    time_column = "timestamp", #,format="%Y-%m-%d %H:%M:%S"),
                    #data=dat, 
  ...
  dat_sf_line <-data_move %>% 
    group_by (trackId) %>% 
    summarise(do_union = FALSE) %>% 
    st_cast("LINESTRING")
  clust_sf <-st_as_sf(clus_summary, 
                      coords=c("g_c_location_long", "g_c_location_lat"), 
                      crs=st_crs(4326))
  ...
  pdf_path <- appArtifactPath(str_interp("kill_cluster_radius_${search_radius}m_search_window_${window_days}d.pdf"))
  
  n_pages <- length(unique(dat_sf_line$trackId)) 
  
  
  plot_list <- list()
  for(i in unique(dat_sf_line$trackId))
  {
    temp_dat <- dat_sf_line |> filter(trackId == i)
    temp_clust <- clust_sf |> filter(trackId == i)
    
    p <- ggplot()+
      geom_sf(data = temp_dat) +
      geom_sf(data = temp_clust, col = "brown", size = 1)+
      annotation_scale(location = "br")+
      annotation_north_arrow(location = "tl", 
                             width = unit(0.6,"cm"),
                             height = unit(1,"cm"))+
      labs(title = paste("Individual_track", i))+
      theme_bw()
    plot_list[[i]] <- p
  }
...
#  Print each plot to a new page
  ...
  # ggsave(map_out_new, width=12, height = 8, units = "in", dpi=300, 
  #           file= paste0(Sys.getenv(x = "APP_ARTIFACTS_DIR", "/tmp/"),"Cluster_locations_plot.jpeg"))
  write.csv(clus_summary, 
            file= paste0(Sys.getenv(x = "APP_ARTIFACTS_DIR", "/tmp/"), "Cluster_summary_output.csv"),
            row.names=FALSE)
